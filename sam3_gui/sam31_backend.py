...
class Sam31SessionNotFound(LookupError):
    """Raised when a request references an unknown SAM 3.1 session."""


class Sam31ObjectNotFound(LookupError):
    """Raised when a request references an unknown object in a known session."""
...
def validate_sam31_checkpoint_path(checkpoint_path: str | None) -> str | None:
    ...
    if (
        "sam3.1" not in checkpoint_name
        or "multiplex" not in checkpoint_name
...
def assert_sam31_available() -> None:
    try:
        ...
        raise RuntimeError(
            ...
            "Update your sam3 checkout and reinstall it with `pip install -e /path/to/sam3`."
        ) from exc


def preflight_fa3() -> None:
    try:
        from sam3.perflib.fa3 import flash_attn_func
    except Exception as exc:
        raise RuntimeError(
            "sam.use_fa3=true requires FlashAttention 3 from the installed sam3 package, "
            "but the FA3 module could not be imported. Reinstall SAM3/FA3 or run with sam.use_fa3=false."
        ) from exc

    if not torch.cuda.is_available():
        raise RuntimeError(
            "sam.use_fa3=true requires CUDA. Run with sam.use_fa3=false on CPU."
        )
    ...
    try:
        ...
        raise RuntimeError(
            "sam.use_fa3=true was requested, but the installed FlashAttention 3 CUDA kernel "
            f"does not run on {device_name} (compute capability {capability[0]}.{capability[1]}). "
            "Rebuild or replace FA3 with kernels for this GPU, or run with sam.use_fa3=false."
        ) from exc
...
def make_predictor_init_state_compatible(predictor: Any) -> None:
    ...
    def compatible_init_state(*args, **kwargs):
        ...
        if offload_state_to_cpu:
            raise TypeError(
                "SAM 3.1 multiplex init_state does not support offload_state_to_cpu=True"
            )
...
def sam31_model_load_error(exc: Exception) -> Sam31ModelLoadError:
    checkpoint_hint = (
        f"Place {SAM31_CHECKPOINT_NAME} under ~/sam3/model, pass sam.checkpoint_path=..., "
        f"authenticate with `hf auth login` for {SAM31_HF_REPO}, "
        "or run `python -m tools.download_model --source modelscope`."
    )
...
def output_masks_by_obj(
    outputs: dict[str, Any] | None, target_shape: tuple[int, int] | None = None
) -> dict[int, np.ndarray]:
    if not outputs:
        return {}
...
def normalize_points(
    points: list[list[float]] | np.ndarray, width: int, height: int
) -> list[list[float]]:
...
def normalize_xyxy_box(
    box_coords: tuple[float, float, float, float], width: int, height: int
) -> list[float]:
...
def serialize_outputs(
    outputs: dict[str, Any] | None, include_masks: bool = True
) -> dict[str, Any]:
    outputs = outputs or {}
    obj_ids = to_numpy(outputs.get("out_obj_ids", []), dtype=np.int64).reshape(-1)
...
def _box_xywh_to_pixels(box: list[float], image_size: tuple[int, int]) -> list[float]:
...
def serialize_instances(
    ...
    *,
    ...
    box_format: Literal["xywh_normalized", "xywh_pixel"] = "xywh_normalized",
    ...
    for idx, obj_id in enumerate(obj_ids):
        box_xywh: list[float] = []
        if boxes.size and idx < len(boxes):
            box_xywh = [float(v) for v in boxes[idx].tolist()]
            if box_format == "xywh_pixel" and image_size is not None and box_xywh:
                box_xywh = _box_xywh_to_pixels(box_xywh, image_size)
        ...
        if include_masks and idx < len(masks):
            instance["mask_rle"] = mask_to_rle(masks[idx])
...
class Sam31Backend:
    ...
    def __init__(self, config: Sam31BackendConfig | None = None):
        ...
        self.config.checkpoint_path = validate_sam31_checkpoint_path(
            self.config.checkpoint_path
        )
        self.predictor = None
        self._active_sessions: set[str] = set()
        self._session_object_counts: dict[str, int] = {}
        self._session_object_ids: dict[str, set[int]] = {}
    ...
    def start_session(self, resource_path: Any, **kwargs) -> str:
        with self._predictor_lock:
            ...
            self._session_object_counts[session_id] = 0
            self._session_object_ids[session_id] = set()
            return session_id

    def _ensure_session(self, session_id: str) -> None:
        if session_id not in self._active_sessions:
            raise Sam31SessionNotFound(f"Session not found: {session_id}")
    ...
    def close_session(self, session_id: str) -> dict[str, Any]:
        with self._predictor_lock:
            ...
            self._session_object_ids.pop(session_id, None)
            return response
    ...
    def add_prompt(self, session_id: str, frame_index: int, **kwargs) -> dict[str, Any]:
        with self._predictor_lock:
            ...
            obj_ids = to_numpy(outputs.get("out_obj_ids", []), dtype=np.int64).reshape(
                -1
            )
            self._session_object_counts[session_id] = int(len(obj_ids))
            self._session_object_ids[session_id] = {
                int(obj_id) for obj_id in obj_ids.tolist()
            }
            return response

    def remove_object(
        self, session_id: str, obj_id: int, frame_index: int = 0
    ) -> dict[str, Any]:
        with self._predictor_lock:
            self._ensure_session(session_id)
            known_obj_ids = self._session_object_ids.get(session_id, set())
            if int(obj_id) not in known_obj_ids:
                raise Sam31ObjectNotFound(
                    f"Object {obj_id} not found in session {session_id}"
                )
            with torch_autocast_context():
                response = self.ensure_predictor().handle_request(
                    request={
    ...
    def propagate(
        ...
        with self._predictor_lock:
            ...
            if self._session_object_counts.get(session_id) == 0:
                return
            with torch_autocast_context():
                yield from self.ensure_predictor().handle_stream_request(
                    request=request
                )
