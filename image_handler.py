...
from utils import get_hls_palette
...
class ImageModeHandler:
    ...
    def set_image(self, image, resource_path=None):
        ...
        if resource_path is not None:
            pil_image = Image.open(resource_path).convert("RGB")
            self.current_image = np.array(pil_image)
            session_resource = resource_path
        elif image is None:
            return None, "No image provided"
        elif isinstance(image, np.ndarray):
            self.current_image = image[:, :, :3] if image.ndim == 3 else np.stack([image] * 3, axis=-1)
            pil_image = Image.fromarray(self.current_image)
            session_resource = [pil_image]
        else:
            pil_image = image.convert("RGB") if isinstance(image, Image.Image) else Image.fromarray(np.array(image))
            self.current_image = np.array(pil_image)
            session_resource = [pil_image]

        self.session_id = self.backend.start_session(session_resource)
        self.current_masks = []
        self.current_scores = []
        self.selected_points = []
        self.selected_labels = []
    ...
    def _set_outputs(self, outputs):
        if self.current_image is None:
            ...
            self.current_scores = []
            return
        masks_by_obj = output_masks_by_obj(outputs, target_shape=self.current_image.shape[:2])
    ...
    def _segment_with_points(self):
        ...
        try:
            ...
            if self.current_masks:
                ...
                return self._visualize_masks(show_points=True), f"Generated {len(self.current_masks)} mask(s){score_msg}"
    ...
    def _visualize_masks(self, show_points=False, mask_idx=0, show_all=False):
        ...
        if self.current_masks:
            ...
            mask_indices = [idx for idx in mask_indices if idx < len(self.current_masks)]
    ...
    def save_mask(self, output_path: str):
        ...
        mask = np.squeeze(self.current_masks[0])
